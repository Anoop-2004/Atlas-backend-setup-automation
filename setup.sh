...
main() {
    ...
    prompt_user_action "PREREQUISITES:\n\n  • Download Docker Desktop and move it to the Applications folder:\n    https://docs.docker.com/desktop/setup/install/mac-install/\n\n  • Request access to 'Hashicorp Okta' and 'Hashicorp Okta Certificate'\n    using IBM AccessHub: https://ibm-support.saviyntcloud.com/ECMv6/request/requestHome\n    Once access is granted, log into Artifactory through your Okta dashboard.\n\n  • Request pull access to the artifactory-users group (also adds the\n    Artifactory tile to Okta):\n    - Go to doormat -> App Access -> Request Access\n    - Choose Artifactory Application from the drop-down menu\n    - Request the artifactory-users group\n    - Provide a justification and click Submit\n\n  • User should have a GitHub account with access to the HashiCorp organization.\n    Once your GitHub account is activated, request access to the following teams:\n    - https://passport.hashicorp.services/namespaces/doormat/groups/github-access-core\n    - https://passport.hashicorp.services/namespaces/doormat/groups/github-hashicorp-team-terraform-enterprise\n\n  • If running the setup from home, ensure VPN is disconnected and IPv6 is\n    disabled: System Settings -> Network -> WiFi -> Details -> TCP/IP\n    Set 'Configure IPv6' to 'Link-local only'.\n    (Re-enable IPv6 once setup is complete.)\n\n${BOLD}Please ensure all prerequisites are met before continuing.\n Note: Access requests may take 2-3 hours to become effective. Run the script\n       2-3 hours after completing the prerequisites. ${RESET}"
